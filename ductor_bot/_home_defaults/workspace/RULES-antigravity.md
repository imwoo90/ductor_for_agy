...
- **Log Watcher Progress:** The bot's Log Watcher (`_run_log_monitor_loop`) automatically parses `transcript.jsonl` and broadcasts updates (`💭 Thinking Process`, `🛠️ Tool Calls`, `📥 Tool Completions`) to Telegram. Once the command finishes and you respond, it pushes the final answer (`✅ Final Response`).
...
### 3. Interactive Tool Questions (No TUI)
- **No Interactive Tool Questions:** Do NOT call the interactive `ask_question` tool. In headless/Telegram environments, it immediately fails due to stdin being redirected. If you need user input, choices, or clarification (e.g., tech stack selections during planning), write your questions and options directly in your text response and exit the turn. The user's reply will be provided as input in the next turn.
